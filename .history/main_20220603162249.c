...
void windowinit(Display *display, Window root, ){
      sizeof feature / sizeof *feature
}
...
int main()
{
  ...
  XResizeWindow(display, windows[k].id, csizex, sizey/2);
  XMoveWindow(display, windows[k].id, px, l);
  XFlush(display);
   ...
   printf("0 pos: %ix%i\n1 pos: %ix%i\n\n0 size: %ix%i\n1 size: %ix%i\n\nssizex:%i\nssizey:%i", windows[j].screenx, windows[j].screeny,windows[k].screenx, windows[k].screeny,windows[j].sizex, windows[j].sizey,windows[k].sizex, windows[k].sizey,sizex,sizey);
   return 0;
