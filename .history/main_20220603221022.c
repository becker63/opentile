...
int display_get_current_destop(Display *display, Window window)
{
  ...
  if (XGetWindowProperty(display, window, XInternAtom(display, "_NET_CURRENT_DESTOP", False), 0, 1,
...
int window_get_destop(Display *display, Window window)
{
  ...
  if (XGetWindowProperty(display, window, XInternAtom(display, "_NET_WM_DESTOP", False), 0, 1, False, XA_CARDINAL, &actual_type_return, &actual_format_return, &nitems_return, &bytes_after_return, (unsigned char **)(void *)&desktop) != Success)
    return 0;
...
int main()
{
  ...
  int desktop = display_get_current_destop(display, root);
