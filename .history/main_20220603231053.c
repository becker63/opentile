...
void printstructs()
{
  ...
  for (a = 0; a < n; a++)
  {
    if (windows[a].id != 0)
    {
      ...
      strcat(formatedname, "...");
      printf(" %i:    %i  *  %i     %i  *  %i    %s    %lu\n", a, windows[a].screenx, windows[a].screeny, windows[a].posx, windows[a].sizey, formatedname, windows[a].id);
    }
  }
}
...
int clostestwindow(int size, Display *display, int gap){
    ...
    for ( i = 0; i != size; i++) {
      if (windows[i].id != 0){
        ...
        if(abs(csizepos - res) > abs(csizepos - windows[i].screenx)){
              res = windows[i].screenx;
              printf("%i  %i\n", res, csizepos);
              numbwindows++;
...
int main()
{
  ...
  int posx = screen->width;
   ...
   printf("\n\n\n\n\n0 pos: %ix%i\n1 pos: %ix%i\n\n0 size: %ix%i\n1 size: %ix%i\n", windows[0].screenx, windows[0].screeny,windows[1].screenx, windows[1].screeny,windows[0].posx, windows[0].sizey,windows[1].posx, windows[1].sizey);
   return 0;
