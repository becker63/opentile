...
int main()
{
  ...
  int csizex = sizex/2 - (h + l);


  printf("%ix%i\n", windows[j].screenx, windows[j].screeny);

  XResizeWindow(display, windows[j].id, csizex, sizey - l*2);
  XMoveWindow(display, windows[j].id, l, l);
  XFlush(display);
  ...
  int k = findwindow("becker");

  XResizeWindow(display, windows[k].id, sizex, sizey/2);
  //XMoveWindow(display, windows[k].id, screen->width/1.967, gap / 2);
  XFlush(display);

  int i = findwindow("becker");

  XResizeWindow(display, windows[i].id, sizex, sizey/2);
  XMoveWindow(display, windows[i].id, screen->width/1.967, screen->height/2);
  XFlush(display);
