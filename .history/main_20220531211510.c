...
int main()
{
  ...
  int sizex = (screen->width / openwindows) - gap;
  ...
  XResizeWindow(display, windows[k].id, (screen->width / openwindows) - gap;, sizey);
  XMoveWindow(display, windows[k].id, screen->width/2, gap / 2);
  XFlush(display);
