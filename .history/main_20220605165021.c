...
int clostestwindow(int size, Display *display, int gap){
    ...
    for ( i = 0; i != size; i++) {
      if (windows[i].id != 0){
        ...
        if (numbwindows != 1){
            cposx =(posx / numbwindows) + gap;
            cposy = (posy / numbwindows) - gap;
        }            
        int slope = windows[i].screenx - cposx / windows[i].screeny - cposy;
        printf("window: %i\nxy: %ix%i\nexpected xy: %ix%i\nslope: %i\nnw: %i\nsxy: %ix%i\n\n", i, windows[i].screenx,windows[i].screeny,cposx,cposy,slope, numbwindows,posx,posy);  
