...
int display_get_current_destop(Display *display, Window window)
{
  ...
  if (XGetWindowProperty(display, window, XInternAtom(display, "_NET_CURRENT_DESKTOP", False), 0, 1,
                         ...
                         &nitems_return, &bytes_after_return, (unsigned char **)&destop) != Success)
  {
    return 0;
  }
...
int window_get_destop(Display *display, Window window)
{
  Atom actual_type_return;
  int actual_format_return = 0;
  unsigned long nitems_return = 0;
  ...
  if (XGetWindowProperty(display, window, XInternAtom(display, "_NET_WM_DESTOP", False), 0, 1, False, XA_CARDINAL, &actual_type_return, &actual_format_return, &nitems_return, &bytes_after_return, (unsigned char **)(void *)&destop) != Success)
    return 0;
...
int display_get_windows(Display *display, Window window, int destop)
{
  Atom actual_type_return;
  int actual_format_return = 0;
  unsigned long bytes_after_return = 0;
  ...
  {
    long w_destop = window_get_destop(display, twindows[count]);
    if (w_destop == destop)
    {
      XTextProperty text;
...
int main()
{
  ...
  int destop = display_get_current_destop(display, root);
  int currentwindows = display_get_windows(display, root, destop);
