...
int main() {
  ...
  for(a = 0; a < n; a++)
     {
        if (windows[a].id != 0){
        char formatedname[10];
        strncpy(formatedname, &windows[a].name[0], 5);
        printf("%i:  %i x %i,   %i x %i    %s\n",a, windows[a].screenx, windows[a].screeny, windows[a].sizex, windows[a].sizey,windows[a].name);
        }
