...
int window_get_desktop(Display * display, Window window) {
	...
	if (XGetWindowProperty(display, window, XInternAtom(display, "_NET_WM_DESKTOP", False), 0, 1, False, XA_CARDINAL, &actual_type_return, &actual_format_return, &nitems_return, &bytes_after_return, (unsigned char**)(vPoid*)&desktop) != Success) return 0;
