...
int display_get_windows(Display* display, Window window, int desktop) {
    ...
    for (count = 0; count < nitems_return; count++) {
		...
		if (w_desktop == desktop) {
            XTextProperty text;
            XGetWMName(display, windows[count], &text);

            int x, y;
            
            XWindowAttributes attr;
            Window child;
            XGetWindowAttributes(display, windows[count], &attr);

            XTranslateCoordinates( display, windows[count], window, 0, 0, &x, &y, &child );

            printf("%s     %d x %d,    %d x %d \n", text.value, attr.width, attr.height, x - attr.x, attr.y);
			x = x + 1;
		}
    }
