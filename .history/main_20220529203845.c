...
int display_get_windows(Display* display, Window window, int desktop) {
    ...
    for (count = 0; count < nitems_return; count++) {
		...
		if (w_desktop == desktop) {
            ...
            printf("%s: %lu     %d x %d,    %d x %d \n", text.value,twindows[count], attr.width, attr.height, x - attr.x,y - attr.y);

            
            windows[x].id = twindows[count];
            windows[x].screenx = x - attr.x;
            windows[x].screeny = y - attr.y;
            windows[x].sizex = attr.width;
            windows[x].sizey = attr.height;
			x = x + 1;
		}
    }
  ...
  for(a = 0; a < n; a++)
     {
        ...
        printf("%i x %i, %i x %i id %lu\n", windows[a].screenx, windows[a].screeny, windows[a].sizex, windows[a].sizey, windows[a].id);
