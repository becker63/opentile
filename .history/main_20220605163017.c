...
void printstructs()
{
  ...
  for (a = 0; a < n; a++)
  {
    if (windows[a].id != 0)
    {
      ...
      strcat(formatedname, "...");
      printf(" %i:    %i  *  %i     %i  *  %i    %s    %lu\n", a, windows[a].screenx, windows[a].posy, windows[a].posx, windows[a].posy, formatedname, windows[a].id);
    }
  }
}
...
//            cposy = (posy / i) - gap2;
//            
        int slope = windows[i].screenx - cposx / windows[i].posy - cposy;
        printf("window: %i\nxy: %ix%i\nexpected xy: %ix%i\nslope: %i\n\n", i, windows[i].screenx,windows[i].posy,cposx,cposy,slope);  
...
int main()
{
  ...
  int posx = screen->width;
   ...
   printf("\n\n\n\n\n0 pos: %ix%i\n1 pos: %ix%i\n\n0 size: %ix%i\n1 size: %ix%i\n", windows[0].screenx, windows[0].posy,windows[1].screenx, windows[1].posy,windows[0].posx, windows[0].posy,windows[1].posx, windows[1].posy);
   return 0;
