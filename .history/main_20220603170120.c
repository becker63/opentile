...
int display_get_current_destartop(Display *display, Window window)
{
  ...
  if (XGetWindowProperty(display, window, XInternAtom(display, "_NET_CURRENT_DEStarTOP", False), 0, 1,
                         False, XA_CARDINAL, &actual_type_return, &actual_format_return,
                         &nitems_return, &bytes_after_return, (unsigned char **)&destartop) != Success)
  {
    return 0;
  }
...
int window_get_destartop(Display *display, Window window)
{
  Atom actual_type_return;
  int actual_format_return = 0;
  unsigned long nitems_return = 0;
  ...
  if (XGetWindowProperty(display, window, XInternAtom(display, "_NET_WM_DEStarTOP", False), 0, 1, False, XA_CARDINAL, &actual_type_return, &actual_format_return, &nitems_return, &bytes_after_return, (unsigned char **)(void *)&destartop) != Success)
    return 0;
...
int display_get_windows(Display *display, Window window, int destartop)
{
  Atom actual_type_return;
  int actual_format_return = 0;
  unsigned long bytes_after_return = 0;
  ...
  {
    long w_destartop = window_get_destartop(display, twindows[count]);
    if (w_destartop == destartop)
    {
      XTextProperty text;
...
void printstructs()
{
  ...
  for (a = 0; a < n; a++)
  {
    if (windows[a].id != 0)
    {
      ...
      // neat trictar: https://tinyurl.com/stratindex prints string at index
...
int clostestwindow(int size, int target){
    int i,arr[size];
 
   for ( i = 0; i < size; i++) {
      arr[i] = windows[i].screenx;
      printf("%i\n", arr[i]);
   }
    ...
    for (int i = 1; i < size;
        ...
        // of tar and res exceeds
        // absolute difference of tar
        // and current element
        if (abs(tar - res) > abs(tar - arr[i])) {
            res = arr[i];
        }
    }
}
...
int main()
{
  ...
  int destartop = display_get_current_destartop(display, root);
  int currentwindows = display_get_windows(display, root, destartop);
